 ...
 *
 * Differences from the Matek H743 reference mcuconf this file is derived from:
 *  - 16 MHz HSE crystal (Matek = 8 MHz): every PLL DIVM is doubled so the
 *    per-PLL reference frequency (and therefore the whole downstream clock
 *    tree, USB 48 MHz, SDMMC, FDCAN, SPI clocks) is bit-identical to the
 *    proven 8 MHz Matek configuration.
 *      PLL1/PLL2 ref = 16 MHz / 2 = 8 MHz   (Matek: 8 MHz / 1)
 *      PLL3      ref = 16 MHz / 4 = 4 MHz   (Matek: 8 MHz / 2)
 *  - RTCPRE = 16 -> 16 MHz / 16 = 1 MHz (Matek used /8 of 8 MHz).
 *  - SD card on SDMMC2 (Matek = SDMMC1).
 *  - PWM TIM2 and TIM15 additionally enabled (14 PWM outputs vs Matek's 12).
 */
 ...
 * DIVM values doubled vs the 8 MHz Matek config so PLL reference inputs
 * (8/8/4 MHz) and all derived clocks are identical.
 */
 ...
 * FDCAN1 (PD0/PD1) for DroneCAN/UAVCAN companion link.
 */
#if USE_CAN1
 ...
 * I2C1 (PB8/PB7), I2C2 (PB10/PB11), I2C4 (PD12/PD13, onboard baro).
 */
#if USE_I2C1
 ...
 * TIM1/TIM5 are used as PWM outputs on this board, so they are not available
 * for input capture. PPM RC input is therefore not configured by default
 * (use SBUS on a UART instead).
 */
 ...
 *   TIM4  : PWM5-6   (M5-M6)
 *   TIM5  : PWM7-8   (M7-M8)
 *   TIM15 : PWM9-10  (M9-M10)
 *   TIM3  : PWM11-13 (M11-M13)
 *   TIM2  : PWM14    (M14)
 */
 ...
 * micro-SD card slot is wired to SDMMC2.
 */
 ...
 * SPI1 = BMI088 IMU, SPI2 = OSD, SPI3 = ICM42688 IMU.
 */
#if USE_SPI1
